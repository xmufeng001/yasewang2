...
var getPlayerinfo = function (userData,callbock) {

    var user_id=userData["user_id"];
    var headers = userInfo.browser_headers;
    headers["Host"] = userData["host"];
    var postDate = {user_id: user_id};
    var uri = "app/menu/playerinfo?cyt=1"
    var requestObject = {
        headers: headers,
        uri: "http://" + headers["Host"] + "/" + uri
    };
    ...
    request.post(requestObject, function (error, response, body) {
            if (!error && response.statusCode == 200) {
                if (body) {
                    var cookieString = userData["cookie"];
                    cookieString = session_id;
                    xmlreader.read(body, function(errors, result){
                        if(errors){
                            console.log(errors)
                        }
                        var user_id = result.response.body.player_info.id.text();
                        var name = result.response.header.your_data.name.text();
                        var town_level = result.response.header.your_data.town_level.text();
                        var gold = result.response.header.your_data.gold.text();
                        var ap = {current: result.response.header.your_data.ap.current.text(), max: result.response.header.your_data.ap.max.text()};
                        ...
                        var friendship_point = result.response.header.your_data.friendship_point.text();
                        var user_id = result.response.body.login.user_id.text();
                        var user = {name: name, town_level: town_level, gold: gold, ap: ap, bc: bc, friendship_point: friendship_point, user_id: user_id, account: account,cookie:cookieString};
                        userInfo.saveOrUpdateUser(user);
                        console.log(userInfo.getUserList());
                    });
...
var player_search = function (userData) {
    var headers = userInfo.browser_headers;
    headers["Host"] = userInfo.game1_host;
                ...
                if (body) {
                    xmlreader.read(body, function(errors, result){
                        ...
                        if(result.response.header.error.code.text()!="0"){
                            console.error(result.response.header.error.code.text().message);
                        }
                        try{
                            var jsonString=fs.readFileSync("./../conf/game1-CBT_lv4UserList.json") ;
...
//                            console.log( userList.at(i).town_level.text());
                            ...
                            fs.writeFileSync("./../conf/game1-CBT_lv4UserList.json", JSON.stringify(lv4userList));
